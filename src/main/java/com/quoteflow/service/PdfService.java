...
public class PdfService {
    ...
    public PdfService(@Value("${app.upload.pdf-dir}") String outputDir) {
        this.outputDir = outputDir;
    }

    //Genere le PDF du devis, l'ecrit sur disque et retourne son chemin absolu.
    // (Signature conservee : appelee par DevisService.creerEtValider)
     
    public String genererPdfDevis(Devis devis) {
        byte[] contenu = genererBytes(devis);
        try {
    ...
    // Genere le PDF en memoire (utile pour une regeneration a la volee). 
    public byte[] genererBytes(Devis devis) {
